...
from commons.services.extraction import extract_features, ExtractionRequest, read_id3_tag, extract_features_with_all_plugins, \
    convert_if_needed
from commons.utils.conversion import seconds_between, object_size_humanized
...
def read_mp3_id3_tag(input_file_name: Text) -> Dict[Text, Any]:
    ...
    input_audio_file_absolute_path = os.path.join(AUDIO_FILES_DIR, input_file_name)
    id3_tag = read_id3_tag(input_audio_file_absolute_path)
    took_seconds = seconds_between(start_time_point=start_point)
