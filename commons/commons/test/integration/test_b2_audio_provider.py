...
class B2AudioProviderIntegrationTest(unittest.TestCase):
    def setUp(self):
        b2_test_config = B2Config(B2_ACCOUNT_ID, B2_APPLICATION_KEY,
                                  B2_RESOURCES_BUCKET)
    ...
    def test_file_should_exists(self):
        test_file_exists = self.audio_provider.exists(B2_TEST_FILE_PATH)
    ...
    def test_remote_file_paths_should_contain_test_file(self):
        ...
        assert_that(remote_file_paths).contains(B2_TEST_FILE_PATH)
    ...
    def test_should_download_remote_file(self):
        local_file_path = self.audio_provider.download(B2_TEST_FILE_PATH)
