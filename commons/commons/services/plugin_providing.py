...
def list_vampy_plugins(blacklisted_plugin_keys: Optional[List[Text]] = None) -> List[VampyPlugin]:
    ...
    return [build_plugin_from_key(key=k) for k in list_plugin_keys(blacklisted_plugin_keys)]


def list_plugin_keys(blacklisted_plugin_keys: Optional[List[Text]] = None) -> List[Text]:
    """Returns list of VAMPy plugin keys available under OS"""
    return [k for k in vamp.list_plugins() if k not in (blacklisted_plugin_keys or [])]
