...
class CoordinatorApiTest(TestCase):
    ...
    def test_should_accept_mp3_task_and_return_extracted_data(self):
        extraction_expected_status_code = 202
        response = requests.post(url=self.extraction_api_url, json=self.mp3_extraction_request)
        assert_that(response.status_code).is_equal_to(extraction_expected_status_code)
        ...
        results_response = requests.get(url="{}/{}".format(self.result_api_url, task_id))
        assert_that(results_response.status_code).is_equal_to(retrieval_expected_status_code)
        ...
        clean_up_response = requests.delete(url="{}/{}".format(self.result_api_url, task_id))
        assert_that(clean_up_response.status_code).is_equal_to(retrieval_expected_status_code)
