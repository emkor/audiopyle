...
def start_app(logger: Logger, host: str, port: int, debug: bool = False):
    ...
    app.add_url_rule("/audio/tmp", view_func=TmpAudioApi.as_view('audio_tmp_api', logger=logger))
    app.add_url_rule("/audio/meta", view_func=AudioMetaApi.as_view('audio_meta_api', logger=logger))
    app.add_url_rule("/audio/tag", view_func=AudioTagApi.as_view('audio_tag_api', logger=logger))
