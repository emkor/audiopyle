...
class LocalAudioMetaProviderIntegrationTest(unittest.TestCase):
    ...
    def test_reading_audio_meta(self):
        ...
        assert_that(audio_meta.filename).is_equal_to(XtracterConst.TEST_WAV_FILE_NAME)
        assert_that(audio_meta.channels_count).is_equal_to(XtracterConst.TEST_WAV_FILE_CHANNELS_COUNT)
        assert_that(audio_meta.bit_depth).is_equal_to(XtracterConst.TEST_WAV_FILE_BIT_DEPTH)
        assert_that(audio_meta.sample_rate).is_equal_to(XtracterConst.TEST_WAV_FILE_SAMPLE_RATE)
        assert_that(audio_meta.frames_count).is_equal_to(XtracterConst.TEST_WAV_FILE_FRAME_COUNT)
