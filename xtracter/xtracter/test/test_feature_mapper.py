...
class FeatureMapperTest(unittest.TestCase):
    def test_should_map_list_type_feature_without_values(self):
        ...
        mapped_features = FeatureMapper.format_dict(list_type_feature)
        assert_that(mapped_features).is_not_empty().is_length(4)
        assert_that(mapped_features[0].get("label")).is_equal_to('some_label')
        assert_that(mapped_features[0].get("timestamp")).is_equal_to(0.0)
        assert_that(mapped_features[1].get("timestamp")).is_equal_to(0.011609977)
    ...
    def test_should_map_vector_type_feature(self):
        ...
        mapped_features = FeatureMapper.format_dict(list_type_feature)
        assert_that(mapped_features).is_not_empty().is_length(4)
        assert_that(mapped_features[1].get("timestamp")).is_equal_to(0.005804988)
        assert_that(mapped_features[2].get("timestamp")).is_equal_to(2 * 0.005804988)
    ...
    def test_should_map_matrix_type_feature(self):
        ...
        assert_that(first_feature.get("value")).is_length(7)
        assert_that(mapped_features[1].get("timestamp")).is_equal_to(0.023219954)
